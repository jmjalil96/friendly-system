...
export interface InsurersListTableProps {
  data: ListInsurersItem[]
  isLoading: boolean
  sorting: SortingState
  onSortingChange: OnChangeFn<SortingState>
  pagination: PaginationState
  onPaginationChange: OnChangeFn<PaginationState>
  paginationMeta?: ListInsurersResponse['meta']
  isError?: boolean
...
export function InsurersListTable({
  data,
  isLoading,
  sorting,
  onSortingChange,
  ...
  onClearFilters,
}: InsurersListTableProps) {
  const columns = useMemo<ColumnDef<ListInsurersItem>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'Aseguradora',
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-[var(--color-gray-700)]">
            {formatInsurerType(row.original.type)}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-xs text-[var(--color-gray-500)]">
            {formatInsurerDateTime(row.original.createdAt, 'date')}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-xs text-[var(--color-gray-500)]">
            {formatInsurerDateTime(row.original.updatedAt, 'date')}
          </span>
        ),
      },
    ],
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            <h3 className="text-sm font-semibold text-[var(--color-gray-900)]">
              No pudimos cargar las aseguradoras
            </h3>
        ...
        <div className="mx-auto flex max-w-md flex-col items-center text-center">
          ...
          <p className="mt-1 text-sm text-[var(--color-gray-600)]">
            {hasActiveFilters
              ...
              : 'Cuando se creen aseguradoras aparecerán aquí.'}
          </p>
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <DataTable
          ...
          data={data}
          isLoading={isLoading}
          emptyMessage="No hay aseguradoras para los filtros seleccionados."
          className="space-y-3"
