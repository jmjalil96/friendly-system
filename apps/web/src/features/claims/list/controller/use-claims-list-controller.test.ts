...
import {
  ...
  type UseClaimsListControllerResult,
...
describe('useClaimsListController', () => {
  let latest: UseClaimsListControllerResult | null = null
  ...
  function Harness({
    ...
    latest = useClaimsListController({
      search,
  ...
  it('does not restore stale debounced search after clear-all sync', async () => {
    ...
    expect(updateSearchMock).toHaveBeenCalledTimes(0)
    expect(latest?.filterBarProps.search).toBe('')
  })
})
