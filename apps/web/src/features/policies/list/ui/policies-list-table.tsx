...
function formatPolicyType(value: ListPoliciesItem['type']) {
...
export interface PoliciesListTableProps {
  data: ListPoliciesItem[]
  isLoading: boolean
  sorting: SortingState
  onSortingChange: OnChangeFn<SortingState>
  pagination: PaginationState
  onPaginationChange: OnChangeFn<PaginationState>
  paginationMeta?: ListPoliciesResponse['meta']
  isError?: boolean
...
export function PoliciesListTable({
  ...
  const columns = useMemo<ColumnDef<ListPoliciesItem>[]>(
    () => [
      {
          ...
          return (
            <span
              className={cn(
                ...
                POLICY_STATUS_BADGE_CLASSNAMES[status],
              )}
            >
      ...
      },
      {
        ...
        header: 'Inicio',
        enableSorting: false,
        cell: ({ row }) => formatPolicyDateOnly(row.original.startDate),
      },
      {
        accessorKey: 'endDate',
        header: 'Fin',
        enableSorting: false,
        cell: ({ row }) => formatPolicyDateOnly(row.original.endDate),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-xs text-[var(--color-gray-500)]">
            {formatPolicyDateTime(row.original.createdAt, 'date')}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-xs text-[var(--color-gray-500)]">
            {formatPolicyDateTime(row.original.updatedAt, 'date')}
          </span>
        ),
      },
    ],
        ...
        <div className="mx-auto flex max-w-md flex-col items-center text-center">
          ...
          <p className="mt-1 text-sm text-[var(--color-gray-600)]">
            {hasActiveFilters
              ...
              : 'Cuando se creen pólizas aparecerán aquí.'}
          </p>
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <DataTable
          ...
          data={data}
          isLoading={isLoading}
          emptyMessage="No hay pólizas para los filtros seleccionados."
          className="space-y-3"
