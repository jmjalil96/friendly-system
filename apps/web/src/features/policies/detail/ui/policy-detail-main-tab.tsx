...
import { usePolicyDetailMainController } from '@/features/policies/detail/controller/use-policy-detail-main-controller'
import { usePolicyWorkflowController } from '@/features/policies/detail/controller/use-policy-workflow-controller'
...
import { PolicyDetailMainLayout } from './policy-detail-main-layout'
import { PolicyDetailMainSections } from './policy-detail-main-sections'
import { PolicyDetailSummaryCard } from './policy-detail-summary-card'

export interface PolicyDetailMainTabProps {
  policyId: string
...
function PolicyDetailMainSkeleton() {
  return (
...
function PolicyDetailMainError({
  onRetry,
}: Pick<PolicyDetailMainTabProps, 'onRetry'>) {
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-[var(--color-gray-600)]">
            Ocurrió un error al obtener el detalle de la póliza.
          </p>
        </div>
      </div>
      <div className="mt-4">
...
export function PolicyDetailMainTab({
  policyId,
  policy,
  isLoading,
  isError,
  onRetry,
}: PolicyDetailMainTabProps) {
  const view = usePolicyDetailMainController({ policyId, policy })
  const workflow = usePolicyWorkflowController({ policyId, policy })

  if (isLoading) return <PolicyDetailMainSkeleton />
  if (isError) return <PolicyDetailMainError onRetry={onRetry} />

  if (!policy) {
    return (
      <div className="rounded-xl border border-[var(--color-gray-200)] bg-white p-6 text-sm text-[var(--color-gray-600)]">
        No encontramos datos de la póliza.
      </div>
    )
  }

  return (
    <PolicyDetailMainLayout
      sections={<PolicyDetailMainSections sections={view.sections} />}
      sidebar={
        <>
