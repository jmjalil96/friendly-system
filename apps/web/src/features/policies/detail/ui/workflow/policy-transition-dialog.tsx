...
export function PolicyTransitionDialog({
  open,
  onOpenChange,
  fromStatusLabel,
  toStatusLabel,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={!isSubmitting}>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="policy-transition-notes"
              className="text-xs font-semibold tracking-wide text-[var(--color-gray-600)] uppercase"
            >
              Notas (opcional)
            </label>
            <Textarea
              id="policy-transition-notes"
              rows={4}
              value={notes}
              disabled={isSubmitting}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isSubmitting}
            className={POLICIES_INLINE_SAVE_BUTTON_CLASSNAME}
          >
            {isSubmitting ? (
